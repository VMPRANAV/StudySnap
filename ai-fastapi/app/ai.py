...
async def index_pdf(
    ...
    *,
    ...
    for idx, chunk in enumerate(chunks):
        ...
        if idx > 0:
            await asyncio.sleep(4.5) # Global rate gate
            
        # Robust Exponential Backoff Retry Loop
        retries = 3
        backoff_delay = 6.0
        vec = None
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt == retries - 1:
                    print(f"[AI][index_pdf] Hard fallback failure at chunk {idx} after {retries} attempts.")
                    raise api_err
                
                print(f"[AI][index_pdf] Error hit: {str(api_err)}. Backing off for {backoff_delay}s (Attempt {attempt + 1}/{retries})...")
                await asyncio.sleep(backoff_delay)
                backoff_delay *= 2.0 # Exponentially scale up the wait time
